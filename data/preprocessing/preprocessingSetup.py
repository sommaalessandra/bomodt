...
def run():
    ...
    #1. Create an additional TAZ file for SUMO simulation
    exportZonesToSUMO(
    ...
    #2. Fill missing direction in the Open Data traffic flow dataset according to convention
    fillMissingDirections(TRAFFIC_FLOW_OPENDATA_FILE_PATH)

    #3. Filter the Open Data traffic flow dataset w.r.t. traffic loop accuracy.
    acceptedAccuracy = 95
    ...
    #4. Adding Zone ID for each measurement inside the input file.
    addZones(inputFilePath=TRAFFIC_FLOW_ACCURATE_FILE_PATH, zoneFilePath=STATISTICAL_AREAS_OPENDATA_FILE_PATH)

    #5. Creating TAZ file for SUMO map starting from open data zone file.
    ...
    #6. Filtering the dataset in a specific TIME WINDOW for testing purposes, and reordering the dataset in a chronological order.
    ...
    #7. Generate correspondence between roadName and edge ID in SUMO net file and creating the SUMO detectors additional file for modeling real induction loop positions in SUMO net.
    ...
    #8. Generate detector additional file for SUMO simulator
    ...
    #10. Generate shadow types for creating road, traffic loop shadows within the DT.
    filterForShadowManager(PROCESSED_TRAFFIC_FLOW_EDGE_FILE_PATH)
    generateRealFlow(PROCESSED_TRAFFIC_FLOW_EDGE_FILE_PATH)

    #11. Generate example edgedata file to be used for route genaration
